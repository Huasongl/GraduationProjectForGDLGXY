...
public class InteractionPresenter {
    ...
    private static void showToast(String message) {
        ArchTaskExecutor.getMainThreadExecutor().execute(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(CommunityApplication.getApplication(), message, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private static boolean isLogin(LifecycleOwner owner, Observer<User> observer) {
        if (UserManager.get().isLogin()) {
            ...
            LiveData<User> liveData = UserManager.get().login(CommunityApplication.getApplication());
            if (owner == null) {
